 ...
 * Admin-configured doctor compensation: percentage of paid revenue, fixed monthly salary, or both (hybrid).

 */



export const defaultFinancialCompensation = () => ({
...
export const normalizeFinancialCompensation = (input = {}) => {
  ...
  /** Synced: any positive % is treated as active (avoids “disabled” when % was set but toggle was off). */
 ...
 * @param {number} paidRevenue - sum of paid appointment amounts (this view / period)

 * @param {object} compensation - normalized or raw from DB

 * @param {{ paidVisitCount?: number }} [options]

 * @returns {{ label: string, doctorAttributed: number, clinicAttributed: number | null, isFixedMonthly: boolean, isHybrid: boolean, revenueSharePart: number, fixedMonthlyPart: number, percentageApplied: number | null, detail: string }}
