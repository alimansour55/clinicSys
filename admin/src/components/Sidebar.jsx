...
const Sidebar = () => {
  ...
  const sidebarWidthClass = 'hidden w-64 md:block'

  return (
    <div
      className={`fixed ${sidebarPosition} top-[4.5rem] bottom-0 z-40 overflow-y-auto border-gray-200 bg-white ${sidebarWidthClass}`}
    >
      {aToken && (
                      ...
                      {section.links.map((link) => {
                        ...
                        const icon =
                          link.path === '/admin-dashboard' ? (
                            <img className='h-5 w-5 shrink-0' src={assets.home_icon} alt='' />
                          ) : (
                            <LinkIcon className={ADMIN_ICON_CLASS} />
                          )
                        return (
                          <li key={link.path}>
                            <StaffNavButton
