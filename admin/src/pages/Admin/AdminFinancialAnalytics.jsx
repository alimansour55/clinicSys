...
const AdminFinancialAnalytics = () => {
  ...
  const previewAttr = useMemo(() => {
    return describeCompensationAttribution(
      ...
      { paidVisitCount: previewPaidVisits }
    )
  }, [previewRevenue, previewPaidVisits, payMode, payPercentage, payFixed])
  ...
  const saveDoctorPay = async () => {
    if (!doctorOptions.length) {
      toast.warn('No doctors in the system')
      return
    }
    if (!payDocId) {
      toast.warn('Choose a doctor')
      return
    }
    setSavingPay(true)
    try {
      ...
      if (payMode === 'hybrid' && pct <= 0 && fix <= 0) {
        toast.warn('Hybrid pay needs a percentage above 0 and/or a fixed monthly amount')
        setSavingPay(false)
        return
      }
      let compPayload
      ...
      toast.success(data.message || 'Pay settings saved')
      await getAllDoctors()
      await load()
    } catch (e) {
      toast.error(e.message)
          ...
          <div className='relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between'>
            <div>
              ...
              <p className='mt-3 max-w-2xl text-sm leading-relaxed text-slate-600 md:text-base'>
                Revenue from <strong>paid patient visits</strong>, receivables, and doctor pay. Configure each doctor with either a{' '}
                <strong>percentage of their paid-visit revenue</strong> or a <strong>fixed monthly salary</strong> — doctors see the
                same numbers on their financial page.
              </p>
            </div>
          ...
          <>
            <section className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6'>
              <div className='rounded-2xl border border-emerald-200/60 bg-gradient-to-br from-white to-emerald-50/90 p-5 shadow-sm xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-emerald-800'>Paid revenue</p>
              ...
              <div className='rounded-2xl border border-amber-200/60 bg-gradient-to-br from-white to-amber-50/90 p-5 shadow-sm xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-amber-900'>Outstanding</p>
                ...
                <p className='mt-2 flex items-center gap-1 text-xs font-medium text-amber-900/80'>
                  ...
                  Unpaid active visits
                </p>
              </div>
              <div className='rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-slate-500'>Paid visits</p>
                <p className='mt-2 text-2xl font-black text-slate-900'>{totals?.paidVisitCount ?? 0}</p>
                <p className='mt-2 text-xs text-slate-500'>Visits counted in paid revenue</p>
              </div>
              <div className='rounded-2xl border border-indigo-200/60 bg-gradient-to-br from-indigo-50 to-white p-5 shadow-sm xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-indigo-900'>% share to doctors</p>
                <p className='mt-2 text-2xl font-black text-indigo-950'>{formatMoney(totals?.revenueShareToDoctors)}</p>
                <p className='mt-2 text-xs text-indigo-800/80'>Share of paid visit revenue (% doctors)</p>
              </div>
              <div className='rounded-2xl border border-violet-200/60 bg-gradient-to-br from-violet-50 to-white p-5 shadow-sm xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-violet-900'>Fixed / month</p>
                <p className='mt-2 text-2xl font-black text-violet-950'>{formatMoney(totals?.fixedMonthlySalarySum)}</p>
                <p className='mt-2 text-xs text-violet-800/80'>Sum of monthly fixed</p>
              </div>
              <div className='rounded-2xl border border-slate-900/10 bg-slate-900 p-5 text-white shadow-lg xl:col-span-1'>
                <p className='text-[11px] font-bold uppercase tracking-wide text-white/70'>Total doctor pay (est.)</p>
                <p className='mt-2 text-2xl font-black'>{formatMoney(totals?.totalDoctorCompensationEstimate)}</p>
                <p className='mt-2 text-xs text-white/65'>% share + fixed salaries</p>
                <p className='mt-3 border-t border-white/10 pt-3 text-[11px] text-white/60'>
                  Clinic from paid revenue (after % only): {formatMoney(estClinicRetains)}
                </p>
              </div>
            </section>
                ...
                <div className='flex flex-col gap-8 lg:flex-row lg:items-stretch lg:justify-between'>
                  <div className='max-w-md shrink-0 lg:border-r lg:border-slate-100 lg:pr-8'>
                    <div className='mb-2 inline-flex items-center gap-2 text-indigo-700'>
                      ...
                      <span className='text-xs font-bold uppercase tracking-wider'>Pay doctor</span>
                    </div>
                    <h2 className='text-xl font-bold text-slate-900 sm:text-2xl'>How this doctor is paid</h2>
                    <p className='mt-3 text-sm leading-relaxed text-slate-600'>
                      <strong>Revenue share:</strong> a percentage of paid visit collections. <strong>Fixed salary:</strong> a set
                      amount per month. <strong>Hybrid:</strong> both at once — the doctor earns the revenue share on paid visits{' '}
                      <em>plus</em> the monthly fixed amount. Changes save to the doctor profile immediately.
                    </p>
                  ...
                  <div className='flex min-w-0 flex-1 flex-col gap-5'>
                    <div className='grid gap-4 sm:grid-cols-3'>
                      <div className='rounded-2xl border border-slate-200 bg-slate-50/90 p-4 text-center shadow-inner'>
                        <p className='text-[10px] font-bold uppercase tracking-wide text-slate-500'>Paid visits</p>
                        <p className='mt-1 text-2xl font-black text-slate-900'>{previewPaidVisits}</p>
                        <p className='mt-1 text-[11px] text-slate-500'>For selected doctor</p>
                      </div>
                      <div className='rounded-2xl border border-emerald-100 bg-emerald-50/80 p-4 text-center shadow-inner'>
                        <p className='text-[10px] font-bold uppercase tracking-wide text-emerald-800'>Paid visit revenue</p>
                        <p className='mt-1 text-lg font-black text-emerald-950'>{formatMoney(previewRevenue)}</p>
                        <p className='mt-1 text-[11px] text-emerald-800/80'>Basis for % mode</p>
                      </div>
                      <div className='rounded-2xl border border-indigo-100 bg-indigo-50/80 p-4 text-center shadow-inner'>
                        <p className='text-[10px] font-bold uppercase tracking-wide text-indigo-900'>Live estimate</p>
                        ...
                        <p className='mt-1 text-[11px] text-indigo-800/80'>
                          {payMode === 'fixed' ? 'Per month' : payMode === 'hybrid' ? '% share + fixed (combined)' : 'From current %'}
                        </p>
                      </div>
                    </div>

                    <div className='rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:p-5'>
                      <label className='mb-2 block text-xs font-bold uppercase text-slate-500'>Doctor</label>
                      <select
                        value={payDocId}
                      ...
                      <p className='mt-4 text-xs font-bold uppercase text-slate-500'>Pay type</p>
                      <div className='mt-2 grid gap-3 sm:grid-cols-3'>
                        ...
                        >
                          <div className='flex items-center gap-2 text-sm font-bold text-slate-900'>
                            ...
                            % of paid visit revenue
                          </div>
                          <p className='mt-1 text-xs leading-relaxed text-slate-600'>Doctor earns a share of money from visits marked paid.</p>
                        </button>
                        <button
                          type='button'
                        ...
                        >
                          ...
                          <p className='mt-1 text-xs leading-relaxed text-slate-600'>Same amount each month, regardless of visit count.</p>
                        </button>
                        <button
                          type='button'
                        ...
                        >
                          ...
                          <p className='mt-1 text-xs leading-relaxed text-slate-600'>Revenue share on paid visits plus a guaranteed monthly amount.</p>
                        </button>
                      </div>
                      ...
                      {payMode === 'percentage' && (
                        <div className='mt-5'>
                          <div className='mb-2 flex flex-wrap items-end justify-between gap-2'>
                            <label className='text-xs font-bold uppercase text-slate-500'>Revenue share (0–100%)</label>
                          ...
                          <div className='flex flex-wrap items-center gap-3'>
                            ...
                            />
                            <span className='text-sm text-slate-600'>of this doctor’s paid visit revenue (see table).</span>
                          </div>
                        </div>
                      )}
                      ...
                      {payMode === 'fixed' && (
                        <div className='mt-5'>
                          ...
                          />
                          <p className='mt-2 text-xs text-slate-500'>Shown to the doctor as their fixed monthly pay.</p>
                        </div>
                      )}
                          ...
                          <div>
                            ...
                            <div className='flex flex-wrap items-center gap-3'>
                              <input
                                ...
                                className='w-24 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-lg font-black text-indigo-900 outline-none focus:ring-2 focus:ring-indigo-500'
                              />
                              <span className='text-sm text-slate-600'>of paid visit revenue (this view).</span>
                            </div>
                          </div>
                          <div>
                            <label className='mb-2 block text-xs font-bold uppercase text-slate-500'>
                            ...
                            />
                            <p className='mt-2 text-xs text-slate-500'>Paid on top of the percentage share from visits.</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className='mt-6 flex flex-col gap-4 rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50/80 to-white p-4 sm:flex-row sm:items-center sm:justify-between'>
                        <div className='flex min-w-0 items-start gap-3'>
                          ...
                          <div className='min-w-0'>
                            <p className='text-xs font-bold uppercase text-indigo-900'>Doctor will see</p>
                            <p className='mt-1 text-sm font-semibold text-indigo-950'>
                              <strong>{formatMoney(previewAttr.doctorAttributed)}</strong>
                              {payMode === 'fixed' ? ' / month' : ` · ${previewAttr.label}`}
                            </p>
            ...
            <section className='overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm'>
              <div className='flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 bg-slate-50/50 px-5 py-4 sm:px-6'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    <h2 className='text-lg font-bold text-slate-900'>Doctor breakdown</h2>
                    <p className='text-xs font-medium text-slate-500'>
                      {doctorId.trim() ? 'Filtered to one doctor' : 'All doctors'} · {displayRows.length} row
                      {displayRows.length === 1 ? '' : 's'} · {totals?.totalAppointments ?? 0} appointments in view
                    </p>
                  </div>
                </div>
                ...
                <table className='min-w-[980px] w-full text-sm'>
                  <thead>
                    <tr className='border-b border-slate-200 bg-slate-50 text-left text-xs font-bold uppercase tracking-wide text-slate-500'>
                      ...
                      <th className='px-4 py-3 text-right'>Paid revenue</th>
                      <th className='px-4 py-3 text-right'>Outstanding</th>
                      <th className='px-4 py-3 text-center'>Paid visits</th>
                      <th className='px-4 py-3 text-center'>All appts</th>
                      <th className='px-4 py-3 text-center'>Rate</th>
                      <th className='px-4 py-3'>Pay model</th>
                      <th className='px-4 py-3 text-right'>Doctor earns (est.)</th>
                      <th className='px-5 py-3 text-right sm:px-6'>Clinic after %</th>
                    </tr>
                  </thead>
                  <tbody>
                    {displayRows.length === 0 ? (
                      ...
                      displayRows.map((r, idx) => (
                        ...
                        >
                          ...
                          <td className='px-4 py-3 text-slate-600'>{r.speciality || '—'}</td>
                          ...
                          <td className='max-w-[220px] px-4 py-3'>
                            ...
                            <p className='mt-1 text-[11px] leading-snug text-slate-500'>{r.compensationDetail}</p>
                          </td>
