...
const MyProfile = () => {
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      {
        isEdit 
        ? <input className='bg-gray-50 text-3xl font-medium w-full max-w-xs mt-4' type="text" value={userData.name} onChange={e => setUserData(prev => ({...prev,name:e.target.value}))} />
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          <p className='text-green-600 font-bold '>{userData.patientId}</p>
      ...
      <div className='mt-10'>
  {
    isEdit 
    ? <button 
        className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-primary flex items-center gap-2'  // ⭐ Add disabled classes
